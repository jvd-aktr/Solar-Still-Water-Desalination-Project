 ...
 * offset_azi_testunit=8;  offset_alt_testunit=9;  (in degree)
 ...
 int PTm=15; //present time in minutes*
 int day_of_year= 161; //day of the year of the experiment*
...
//---------UNIT#3-------------
  ...
  double theta3= atan(h/d3); //elevation angle focus point (in radian)
  ...
  int offset_azi3=5+6; //ofset of azimuth adjustment for unit#3
...
//--------------------(3)------------------------
      ...
      phi3i= 0.5*acos(sin(theta3)*sin(alpha)+cos(theta3)*sin(psi3)*cos(alpha)*sin(beta)+cos(theta3)*cos(psi3)*cos(alpha)*cos(beta));
                           //solar incidence angle (in radian) of unit#3 Guo2011
