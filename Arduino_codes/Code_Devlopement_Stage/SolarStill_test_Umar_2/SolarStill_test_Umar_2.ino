...
/*july 02 2021 - > day_of_year= 183*/
const unsigned char PTh = 17; //present time in hours*
const unsigned char PTm = 5; //present time in minutes*
const unsigned int day_of_year = 183; //day of the year of the experiment*

const unsigned char NumOfMirrorUnits = 11; //number of total mirror units

const unsigned char mirror_alt_offset[NumOfMirrorUnits] = {0}; //mirror altitude angle offset
const unsigned char mirror_azi_offset[NumOfMirrorUnits] = {0}; //mirror azimuth angle offset
...
//---------------UNIT#3-----------------
//------Focus_Point--------
double mirror_pos_x[NumOfMirrorUnits] = { -13.5, 13.5, 0, 0, 13.5, -13.5, 18.75, 18.75, 28.3125, 28.3125}; //x position (in inch) of unit#3
double mirror_pos_y[NumOfMirrorUnits] = { -28.3125, -28.3125, -18.5, 18.5, 28.3125, 28.3125, 9.375, -9.375, -18.9357, 18.9357}; //y position (in inch) of unit#3
double mirror_pos_z[NumOfMirrorUnits] = {0}; //z position (in inch) of unit#3
...
void loop()
{
  ...
  Serial.print("---> \n Current Time (in minutes) = "); Serial.print(t_CT); Serial.print("\t Solar Noon = "); Serial.println(snoon);
  ...
  Serial.print("Sr.\t\t"); Serial.print("Altitude\t"); Serial.print("Azimuth\t\t\n");
  for (int i = 0; i < NumOfMirrorUnits; i++)
  {
    Serial.print(i + 1); Serial.print("\t\t"); Serial.print(mirror_alt[i]); Serial.print("\t\t"); Serial.print(mirror_azi[i]); Serial.print("\t\t\n");
  }
  delay(5000);
...
void Mirror_angles(double _sx, double _sy, double _sz, int _mindex)
{
  ...
  double _fz = (h - mirror_pos_z[_mindex]) / (sqrt(mirror_pos_x[_mindex] * mirror_pos_x[_mindex] + mirror_pos_y[_mindex] * mirror_pos_y[_mindex] + mirror_pos_z[_mindex] * mirror_pos_z[_mindex])); //z-componenet of target_vector number (_mindex + 1)
