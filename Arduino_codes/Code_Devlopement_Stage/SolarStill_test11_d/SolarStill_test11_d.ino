...
//---------UNIT#3-------------
  ...
  double theta3_d= (180/pi)*atan(h/d3); //angle in degree

  double a3R= (pi/180)*(theta3_d+alpha_d)/2; //rotation angle (in radian) for unit#3 (Hnin Wah paper)
  double a3R_d= (180/pi)*a3R; //rirtion angle (in degree) for unit#3
  double phi3i_d= (180/pi)*acos((sin(lat)*sin(decl)*cos(a3R))-(cos(lat)*sin(decl)*sin(a3R)*cos((pi/180)*beta_d))
                  +(cos(lat)*cos(decl)*cos(ha)*cos(a3R))+(sin(lat)*cos(decl)*cos(ha)*sin(a3R)*cos((pi/180)*beta_d))
                  +(cos(decl)*sin(ha)*sin(a3R)*sin((pi/180)*beta))); //solar incidence angle of unit#3

  int a3_d= alpha_d+phi3i_d; //intial angle for altitude adjustment of unit#3
...
//--------------------(3)------------------------
      ...
      a3R= (pi/180)*(theta3_d+alpha_d)/2; //rotation angle (in radian) for unit#3 (Hnin Wah paper)
      a3R_d= (180/pi)*a3R; //rirtion angle (in degree) for unit#3
      phi3i_d= (180/pi)*acos((sin(lat)*sin(decl)*cos(a3R))-(cos(lat)*sin(decl)*sin(a3R)*cos((pi/180)*beta_d))
                  +(cos(lat)*cos(decl)*cos(ha)*cos(a3R))+(sin(lat)*cos(decl)*cos(ha)*sin(a3R)*cos((pi/180)*beta_d))
                  +(cos(decl)*sin(ha)*sin(a3R)*sin((pi/180)*beta))); //solar incidence angle (in degree) of unit#3
     
                  
   if(myTime - last_seen > 100000)
    {
      a3_d= alpha_d+phi3i_d; //intial angle for altitude adjustment of unit#3
